...
for sibling in parser json support; do
...
#
# Only this crate's entry is asserted. A sibling's entry legitimately
...
# exemption: the recorded version of each sibling crate (the engine, the
# json grammar, the support crate). Those entries legitimately move whenever
# the sibling checkout does, and exempting exactly them is what makes a full
# comparison usable here when blanket `--locked` is not.
...
# resolved from checkouts of MAIN. So the day the engine bumps its crate
...
# fails this gate even when every file here is clean. Plain `--check`
# still fails on a dirty file in THIS crate.
"${CARGO[@]}" fmt --check
