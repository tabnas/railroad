...
`ts/doc/` and `go/doc/`, and the four package READMEs (root, `ts/`,
`go/`, `rs/`). This repository has no language-neutral `doc/`
directory. The agent guides (`AGENTS.md` and the nested `AGENTS.md`
files) are working documents, and they are out.
...
a clean run over the gated set: 432 alerts across 12 files. Those
